...
SELECT  EXTRACT(YEAR FROM date) AS year ,EXTRACT(MONTH FROM date) AS month, SUM(amount) AS total_amount_per_month
FROM spends
GROUP BY year, month;
...
SELECT  s.supplier_id, sup.supplier, SUM(s.amount) AS total_amount
FROM spends AS s
INNER JOIN suppliers AS sup
ON s.supplier_id = sup.id
GROUP BY supplier_id, sup.supplier
ORDER BY total_amount DESC;
...
VALUES (DEFAULT, 'Hardware');
INSERT INTO expense_areas
VALUES (DEFAULT, 'IT');
INSERT INTO suppliers
VALUES(DEFAULT, 'Dell');
INSERT INTO spends
VALUES(DEFAULT, (SELECT id FROM expense_types WHERE expense_type = 'Hardware'), (SELECT id FROM expense_areas WHERE expense_area = 'IT'), (SELECT id FROM suppliers WHERE supplier = 'Dell'), '2021-08-19', 38104091, 3780119655, 'Computer Hardware Dell', 32000);
