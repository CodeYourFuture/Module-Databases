...
sum(amount) as total_amount_spent_monthly
from spends
group by month
order by month asc ;
```
...
```sql
select supplier_id , sum(amount) as amount_per_supplier from spends group by supplier_id order by supplier_id;
```
...
```sql
select su.supplier, sum(s.amount) as amount_per_supplier
from suppliers su
inner join spends s on su.id=s.supplier_id
group by supplier
order by supplier;
```
...
```sql
 select date , sum(amount) as amount_per_month
 from spends
