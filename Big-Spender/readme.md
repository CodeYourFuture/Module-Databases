...
```sql
SELECT * FROM spends s JOIN expense_areas e ON (s.expense_area_id = e.id) WHERE e.expense_area = 'Better Hospital Food';
```
...
```sql
SELECT supplier_id, sum(amount) AS total FROM spends GROUP BY supplier_id;
```
...
```sql
SELECT s.supplier_id, sup.supplier, sum(s.amount) AS total FROM spends s JOIN suppliers sup ON (s.supplier_id = sup.id) GROUP BY s.supplier_id, sup.supplier;
```
