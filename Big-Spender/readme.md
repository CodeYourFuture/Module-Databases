...
```sql
INSERT YOUR QUERY HERE: SELECT * FROM spends WHERE amount between 30000 AND 31000;
```
...
```sql
INSERT YOUR QUERY HERE: SELECT * FROM spends WHERE description ilike '%fee%';
```
...
```sql
INSERT YOUR QUERY HERE:  SELECT e.expense_area, s.amount, s.transaction_no FROM spends s JOIN expense_areas e ON e.id = s.expense_area_id WHERE e.expense_area = 'Better Hospital Food';
```
...
```sql
CREATE YOUR QUERY HERE:  SELECT sum(amount) AS total_monthly_amount, date AS month FROM spends GROUP BY month;
```
...
```sql
INSERT YOUR QUERY HERE : SELECT supplier_id, sum(amount) FROM spends GROUP BY 1;
```
...
```sql
INSERT YOUR QUERY HERE : SELECT su.supplier, sum(s.amount) FROM spends s JOIN suppliers su ON s.supplier_id = su.id GROUP BY su.supplier ;
```
...
```sql
CREATE YOUR QUERY HERE : SELECT su.supplier, sum(s.amount), date FROM spends s JOIN suppliers su ON s.supplier_id = su.id WHERE date IN ('2021-03-01', '2021-04-01')  GROUP BY su.supplier, date ORDER BY supplier;
```
...
```sql
INSERT YOUR QUERIES HERE : INSERT INTO expense_types(expense_type) VALUES('Hardware');
...
SELECT * FROM spends WHERE expense_type_id = 42;
