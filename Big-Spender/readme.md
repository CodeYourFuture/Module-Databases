...
```sql
select * from spends where description like '%fee%' or description like '%FEE%' or description like '%Fee%';
```
...
select *
from spends s join
expense_areas e on (s.expense_area_id = e.id)
...
```sql
select su.supplier, sum(sp.amount) from spends sp
join suppliers su
on (sp.supplier_id = su.id)
group by supplier;
```
...
insert into spends (date, transaction_no, supplier_inv_no, amount, description, supplier_id, expense_type_id, expense_area_id)
values ('2021-08-19', 38104091, 3780119655, 32000, 'Computer Hardware Dell', (select id from suppliers where supplier = 'Dell'), (select id from expense_types where expense_type = 'Hardware'), (select id from expense_areas where expense_area = 'IT'));
```
