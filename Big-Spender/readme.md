...
```sql
select * from spends s inner join expense_areas e on (s.expense_area_id = e.id) where expense_area='Better Hospital Food';
```
...
```sql
select to_char(date, 'YYYY-MM') as month, sum(amount) as total from spends group by month;
```
...
```sql
select supplier_id, sum(amount) from spends group by supplier_id;
```
