...
```sql
select * from spends s inner join expense_areas ea on (s.expense_area_id = ea.id) WHERE expense_area = 'Better Hospital Food';
```
...
```sql
select date as Month, SUM(amount) as total_spent from spends group by date;
```
...
```sql
select suppliers.id, SUM(spends.amount) as total_spent from suppliers inner join spends on suppliers.id = spends.supplier_id group by suppliers.id;
```
...
```sql
select supplier as supplier_name, SUM(spends.amount) as total_spent from suppliers inner join spends on suppliers.id = spends.supplier_id group by suppliers.id;
```
...
```sql
select date as Month, SUM(amount) as total_spent from spends where date between '2021-03-01' and '2021-04-01' group by date;
```
